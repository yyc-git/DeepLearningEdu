  ...
  let previousLayerDeltaMap =
    ArraySt.range(0, state.filterNumber - 1)->ArraySt.reduceOneParam(
      (. previousLayerDeltaMap, filterIndex) => {
        ...
        // let previousLayerDeltaMap = NP.addMatrixMap(
        NP.addMatrixMap(
        ...
        // previousLayerDeltaMap->ImmutableSparseMap.mapi((. previousLayerDelta, depthIndex) => {
        //   NP.dot(
        //     previousLayerDelta,
        //     previousLayerNets
...
> = (
  ...
  let previousLayerDeltaMap = bpDelta(
    previousLayerActivatorData,
  ...
  (previousLayerDeltaMap, gradientData->Some)
}
