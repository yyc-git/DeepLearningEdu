...
function bpDelta(param, param$1, layerDelta, state) {
  var inputHeight = state.inputHeight;
  var inputWidth = state.inputWidth;
  if (Vector$Cnn.length(layerDelta) === Math.imul(Math.imul(state.depthNumber, inputWidth), inputHeight)) {
    
  } else {
    Exception$Cnn.throwErr("error");
  }
  var oneDeltaMapCount = Math.imul(inputWidth, inputHeight);
  var match = Vector$Cnn.reducei(layerDelta, (function (param, value, index) {
