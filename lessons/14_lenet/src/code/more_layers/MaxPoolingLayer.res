...
let bpDelta: LayerAbstractType.bpDelta<MatrixMap.t, MatrixMap.t> = (
  ...
  ->ArraySt.reduceOneParam((. previousLayerDeltaMap, depthIndex) => {
    let input = previousLayerOutput->ImmutableSparseMap.getExn(depthIndex)

    let previousLayerDelta = previousLayerDeltaMap->ImmutableSparseMap.getExn(depthIndex)

    let previousLayerDelta =
      ArraySt.range(0, outputRow - 1)->ArraySt.reduceOneParam((. previousLayerDelta, rowIndex) => {
        ArraySt.range(0, outputCol - 1)->ArraySt.reduceOneParam(
          (. previousLayerDelta, colIndex) => {
            ...
            previousLayerDelta->MatrixUtils.setValue(
    ...
    previousLayerDeltaMap->ImmutableSparseMap.set(depthIndex, previousLayerDelta)
