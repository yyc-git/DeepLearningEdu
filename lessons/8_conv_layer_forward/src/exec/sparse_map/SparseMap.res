...
let length = map => map->Js.Array.length

let forEachi = (map, func) =>
  map->Js.Array.forEachi((value, i) => value->SparseMapType.nullableToNotNullable->func(_, i), _)
...
let mapi = (map, func) => map->Js.Array.mapi((value, i) =>
    if NullUtils.isNotInMap(value) {
      Js.Nullable.undefined
    } else {
      func(. value->SparseMapType.nullableToNotNullable, i)->SparseMapType.notNullableToNullable
    }
  , _)
