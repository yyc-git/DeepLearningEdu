...
let forEachMatrix = ((row, col, data) as matrix, func) => {
  ArraySt.range(0, row - 1)->ArraySt.forEach(rowIndex => {
    ArraySt.range(0, col - 1)->ArraySt.forEach(colIndex => {
      func(MatrixUtils.getValue(matrix, rowIndex, colIndex), rowIndex, colIndex)
    })
  })
}
...
let reduceMatrix = ((row, col, data) as matrix, func, initialValue) => {
  ...
  ArraySt.range(0, row - 1)->ArraySt.forEach(rowIndex => {
    ArraySt.range(0, col - 1)->ArraySt.forEach(colIndex => {
      value :=
        func(value.contents, MatrixUtils.getValue(matrix, rowIndex, colIndex), rowIndex, colIndex)
    })
  })
...
let _getMinNumber = %raw(` () => { return -Number.MAX_VALUE } `)
...
let getMatrixMapValue = (matrixMap, lengthIndex, rowIndex, colIndex) => {
  matrixMap->ImmutableSparseMap.getExn(lengthIndex)->MatrixUtils.getValue(rowIndex, colIndex)
}
