...
let train = (state: state, features: array<feature>, labels: array<label>): state => {
  ...
  ArraySt.range(0, epochs - 1)->ArraySt.reduceOneParam((. state, epoch) => {
    let state = features->ArraySt.reduceOneParami((. state, feature, i) => {
      ...
      // Neuron o5
      let d_y5_d_w53 = y3 *. _deriv_linear(net5)
      let d_y5_d_w54 = y4 *. _deriv_linear(net5)
      let d_y5_d_b5 = _deriv_linear(net5)

      let d_y5_d_y3 = state.weight53 *. _deriv_linear(net5)
      let d_y5_d_y4 = state.weight54 *. _deriv_linear(net5)

      // Neuron o3
      let d_y3_d_w31 = x1 *. _deriv_linear(net3)
      let d_y3_d_w32 = x2 *. _deriv_linear(net3)
      let d_y3_d_b3 = _deriv_linear(net3)

      // Neuron o4
      let d_y4_d_w41 = x1 *. _deriv_linear(net4)
      let d_y4_d_w42 = x2 *. _deriv_linear(net4)
