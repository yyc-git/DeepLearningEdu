...
function _handleInputVectorToAvoidTooLargeForSigmoid(max, inputVector) {
  return Vector$Gender_analyze.map(inputVector, (function (param) {
                return _handleInputValueToAvoidTooLargeForSigmoid(max, param);
              }));
}

function _activate_sigmoid_value(x) {
  DebugUtils$Gender_analyze.checkSigmoidInputTooLarge(x);
  return 1 / (1 + Math.exp(-x));
}

function _activate_sigmoid(handleInputVectorToAvoidTooLargeForSigmoid, net) {
  return Vector$Gender_analyze.map(Curry._1(handleInputVectorToAvoidTooLargeForSigmoid, net), _activate_sigmoid_value);
}

function _deriv_sigmoid(handleInputValueToAvoidTooLargeForSigmoid, x) {
  var fx = _activate_sigmoid_value(Curry._1(handleInputValueToAvoidTooLargeForSigmoid, x));
  return fx * (1 - fx);
}

function _activate_linear(net) {
  return net;
}
...
function _activate_softmax(net) {
  return Vector$Gender_analyze.map(net, (function (netValue) {
                return Math.exp(netValue) / Vector$Gender_analyze.reducei(net, (function (sum, netValue, i) {
...
function _forwardLayer2(activate, inputVector, state) {
  ...
  var layerOutputVector = Curry._1(activate, layerNet);
  return [
          layerNet,
          layerOutputVector
        ];
...
var _computeDeltaForCrossEntropyLoss = Vector$Gender_analyze.sub;

function backward(param, n, labelVector, inputVector, state) {
  var match = param[0];
  var layer3Delta = Vector$Gender_analyze.sub(param[1][1], labelVector);
...
function _computeCrossEntroyLoss(labelVector, outputVector) {
  return -Vector$Gender_analyze.dot(labelVector, Vector$Gender_analyze.map(outputVector, (function (prim) {
...
function train(state, sampleCount) {
  ...
  return ArraySt$Gender_analyze.reduceOneParam(ArraySt$Gender_analyze.range(0, 49), (function (state, epoch) {
                ...
                var match = ArraySt$Gender_analyze.reduceOneParami(features, (function (param, feature, i) {
                        ...
                        var layer3Gradient = match$2[1];
                        DebugUtils$Gender_analyze.checkGradientExplosionOrDisappear(Matrix$Gender_analyze.multiplyScalar(10.0, layer3Gradient));
                        return [
                                {
                                  wMatrixBetweenLayer1Layer2: Matrix$Gender_analyze.sub(state.wMatrixBetweenLayer1Layer2, Matrix$Gender_analyze.multiplyScalar(10.0, match$2[0])),
                ...
                console.log([
                      "loss:",
                      match$1[1] / sampleCount
                    ]);
                console.log([
                      "getCorrectRate:",
                      _getCorrectRate(match$2[0], match$2[1])
                    ]);
                return match[0];
              }), state);
}
...
function _emptyHandleInputVectorToAvoidTooLargeForSigmoid(inputVector) {
  return inputVector;
}
...
function checkGradient(inputVector, labelVector) {
  ...
  var match = forward([
        (function (param) {
            return Vector$Gender_analyze.map(param, _activate_sigmoid_value);
          }),
        (function (param) {
            return Vector$Gender_analyze.map(param, _activate_sigmoid_value);
          })
...
exports._handleInputVectorToAvoidTooLargeForSigmoid = _handleInputVectorToAvoidTooLargeForSigmoid;
exports._activate_sigmoid_value = _activate_sigmoid_value;
...
exports._activate_softmax = _activate_softmax;
...
exports._computeDeltaForCrossEntropyLoss = _computeDeltaForCrossEntropyLoss;
...
exports._computeCrossEntroyLoss = _computeCrossEntroyLoss;
...
exports._emptyHandleInputVectorToAvoidTooLargeForSigmoid = _emptyHandleInputVectorToAvoidTooLargeForSigmoid;
