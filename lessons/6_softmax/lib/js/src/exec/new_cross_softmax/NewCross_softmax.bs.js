...
function _computeDeltaForCrossEntropyLoss(outputVector, labelVector) {
  return 1;
}
...
function backward(param, n, labelVector, inputVector, state) {
  ...
  var layer3Gradient = Matrix$Gender_analyze.multiply(Matrix$Gender_analyze.create(Vector$Gender_analyze.length(1), 1, 1), Matrix$Gender_analyze.create(1, Vector$Gender_analyze.length(layer2OutputVector), layer2OutputVector));
  return [
          layer2Gradient,
          layer3Gradient
        ];
