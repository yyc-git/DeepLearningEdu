...
let checkGradient = (inputVector: Vector.t, labelVector: Vector.t): unit => {
  ...
  /* ! forward + backward to compute actual gradient */

  let state = createState(2, 2, 1)

  let ((_, layer2OutputVector), (layer3Net, layer3OutputVector)) as forwardOutput = forward(
    inputVector,
    state,
  )
  ...
  let (layer2Gradient, layer3Gradient) =
    forwardOutput->backward(n, labelVector->Vector.getExn(0), inputVector, state)

  let actualGradient = layer3Gradient->MatrixUtils.getValue(0, 0, _)

  /* ! update weight by add */
  let rowIndex = 0
  let colIndex = 0

  let wMatrix = state.wMatrixBetweenLayer2Layer3
  ...
  let newState1 = {
    ...state,
    wMatrixBetweenLayer2Layer3: (row, col, data),
  }
  ...
  let error1 = _computeErrorForLayer3(labelVector, layer3OutputVector)
  ...
  let error2 = _computeErrorForLayer3(labelVector, layer3OutputVector)
