...
let bpDeltaMap = (
  state,
  inputs: ImmutableSparseMapType.t<depthIndex, Matrix.t>,
  nextLayerDeltaMap: ImmutableSparseMapType.t<depthIndex, Matrix.t>,
): ImmutableSparseMapType.t<depthIndex, Matrix.t> => {
  let outputRow = state.outputHeight
  let outputCol = state.outputWidth

  ArraySt.range(0, state.depthNumber - 1)->ArraySt.reduceOneParam(
    (. currentLayerDeltaMap, depthIndex) => {
      let input = inputs->ImmutableSparseMap.getExn(depthIndex)
      ...
      let currentLayerDelta =
        ArraySt.range(0, outputRow - 1)->ArraySt.reduceOneParam((. currentLayerDelta, rowIndex) => {
          ArraySt.range(0, outputCol - 1)->ArraySt.reduceOneParam((. currentLayerDelta, colIndex) => {
            ...
            currentLayerDelta->MatrixUtils.setValue(
              nextLayerDeltaMap->NP.getMatrixMapValue(depthIndex, rowIndex, colIndex),
    ...
    LayerUtils.createCurrentLayerDeltaMap((state.depthNumber, state.inputWidth, state.inputHeight)),
  )
